...
## load source data
#' Check GA Data
#' A function to check if data exists in the directory
#' If not, it downloads from GCS
#' @param ga_data_file STRING of file path
#' 
#' @return
check_ga_data <- function(ga_data_file) {
  message(sprintf("[-] checking if %s exists...", source_data_file))
  if(!file.exists(ga_data_file)){
    message("[!] no data exists!")
    message("[?] downloading from GCS...")
    source("../extract.R")
  } else {
    message("[!] yes we have data!")
  }
}
check_ga_data(ga_data_file = "../data/viewId_demo.rds")
...
renderValueBox({
  valueBox(
    value = comma(length(unique(selected_data()$sessionFirstArticleTitle))),
    icon = "fa-area-chart"
  )
})
```
...
renderValueBox({
  valueBox(
    value = comma(sum(selected_data()$uniquePageviews)),
    icon = "fa-area-chart"
  )
})
```
...
renderValueBox({
  valueBox(
    value = comma(sum(selected_data()$sessions)),
    icon = "fa-area-chart"
  )
})
```
...
renderValueBox({
  valueBox(
    value = comma(sum(selected_data()$events)),
    icon = "fa-area-chart"
  )
})
```
...
renderValueBox({
  valueBox(
    value = percent(sum(selected_data()$events) / sum(selected_data()$sessions)),
    icon = "fa-area-chart"
  )
})
```
...
renderValueBox({
  valueBox(
    value = comma(sum(selected_data()$transactions)),
    icon = "fa-area-chart"
  )
})
```
...
renderValueBox({
  valueBox(
    value = percent(sum(selected_data()$transactions) / sum(selected_data()$sessions)),
    icon = "fa-area-chart"
  )
})
```
...
renderValueBox({
  valueBox(
    value = custom_dollar_format(sum(selected_data()$revenue)),
    icon = "fa-area-chart"
  )
})
```
